      ...
      if (incX == 1 && incY == 1) {
            ...
            if (m != 0) {
                  for (uint64_t i = 0; i < m; i++) {
                        HY[i] = f16_add(HY[i], f16_mul(HA, HX[i]));
                  }
            }
            if (N < 4) return;
            uint64_t mp1 = m + 1;
            for (uint64_t i = mp1; i < N; i += 4) {
                  HY[i] = f16_add(HY[i], f16_mul(HA, HX[i]));
                  HY[i+1] = f16_add(HY[i+1], f16_mul(HA, HX[i+1]));
                  HY[i+2] = f16_add(HY[i+2], f16_mul(HA, HX[i+2]));
                  HY[i+3] = f16_add(HY[i+3], f16_mul(HA, HX[i+3]));
            }
      } else {
            uint64_t iX = 0;
            uint64_t iY = 0;
